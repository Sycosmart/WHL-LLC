...
Both the homepage's featured gallery and the full catalog on
...
| `featured`    | No       | Set to `true` to show this product in the homepage's featured gallery (up to 5 shown). Omit or set `false` to keep it catalog-only. |

## Adding a New Product

1. **Add the photo**: drop an image file into `assets/images/products/`
   (JPG, PNG, or WEBP; roughly 800x600px works well, matching the
